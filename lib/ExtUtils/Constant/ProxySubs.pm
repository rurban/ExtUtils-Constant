...
B<Warning>: Code generated via ProxySubs with any of the options will
only work for newer Perl versions and thus should not be used for CPAN
modules.  autoload, push or croak_on_error can only be used with perl
5.8 or newer, croak_on_read 5.24.
...
Using any of the option is not yet recommended as it generates code,
which is NOT recommended for CPAN modules.
...
This option creates code usable only since Perl 5.24.
